...
**Net effect on the doctrine above:** mechanism 1 was a real, general bug (bench members could pick up a moveset tie-break from a completely unrelated scoring battle) and is now fixed. Mechanism 2's `.battle`/`baitShields`/`farmEnergy`/`priority` leak was also real and is now fixed, and measurably changes some outcomes -- but the specific knife-edge flip this section documents survives it, so the serial/threaded HP-margin and knife-edge-battle drift documented above and in T15c's correction is **still not fully explained or resolved**; the serial-reference doctrine stands. Re-running `scripts/variance-study.mjs --candidates 5 --opponents 8 --shuffles 3 --seed variance-study` before and after the ORIGINAL mechanism-1 fix, on that fire's repo state, had produced BIT-IDENTICAL output (same 1-flip-out-of-4-orderings result) -- consistent with mechanism 1 depending on what a completely separate earlier scoring pass leaves lying around, not on battle order within the study itself. See ROADMAP.md's TrainingAI variance study entry and GOALS.md's T20b sub-bullet for the full numbers.
