...
### Building the CSV from a screen recording

The video scanner lives in its own repository:
**[Pokemon GO Video-to-CSV](https://github.com/Gidntsquia/pokemon-go-video-to-csv)**.
If you'd rather not keep a Poke Genie export up to date, it turns a screen
recording of your box (macOS only — swipe through with the appraisal panel
open) into the collection CSV this pipeline reads:

```bash
# in the Pokemon-GO-Video-to-CSV repo:
node scripts/scan-video.mjs my-box.mp4 --out scanned.csv
# then in this one:
node src/cli.js scanned.csv
```

See that repo's README for what it reads, how it verifies every row against
pvpoke's CP math, and its limitations (it cannot see moves, Lucky or Best
Buddy status).
...
- The video scanner (now its own repo,
  [Pokemon GO Video-to-CSV](https://github.com/Gidntsquia/pokemon-go-video-to-csv))
  is macOS-only and cannot see a Pokemon's moves, Lucky or Best Buddy status.
